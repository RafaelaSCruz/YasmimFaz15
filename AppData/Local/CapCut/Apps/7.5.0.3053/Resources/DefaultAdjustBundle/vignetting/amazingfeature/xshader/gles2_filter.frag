 ...
 * 
 * @param color RGB color vector (each component in range [0,1]).
 * @return HSL color vector where:
 *         - x = hue (0.0 to 1.0)
 *         - y = saturation (0.0 to 1.0)
 *         - z = lightness (0.0 to 1.0)
 */
vec3 RGBToHSL(vec3 color) {
    vec3 hsl;
    float fmin = min(min(color.r, color.g), color.b); // Minimum RGB component
    float fmax = max(max(color.r, color.g), color.b); // Maximum RGB component
    float delta = fmax - fmin;                        // Delta between max and min
    
    hsl.z = (fmax + fmin) / 2.0;                      // Lightness
    
    // Achromatic color (gray) case
    if (delta == 0.0) {
        hsl.x = 0.0;                                  // Hue undefined, set to 0
        hsl.y = 0.0;                                  // Saturation 0
    }
    // Chromatic color case
    else {
...
/**
 * @brief Helper function for HSL to RGB conversion. Converts a hue value to an RGB component.
 * 
 * @param f1 First intermediate value from HSL conversion.
 * @param f2 Second intermediate value from HSL conversion.
 * @param hue Normalized hue value (0.0 to 1.0).
 * @return RGB component value (0.0 to 1.0).
 */
float HueToRGB(float f1, float f2, float hue) {
    ...
    float res;
    // Calculate RGB component based on hue segment
    if ((6.0 * hue) < 1.0)
...
/**
 * @brief Converts an HSL color to RGB color space.
 * 
 * @param hsl HSL color vector where:
 *            - x = hue (0.0 to 1.0)
 *            - y = saturation (0.0 to 1.0)
 *            - z = lightness (0.0 to 1.0)
 * @return RGB color vector (each component in range [0,1]).
 */
vec3 HSLToRGB(vec3 hsl) {
    ...
    // Achromatic color (gray) case
    if (hsl.y == 0.0)
        rgb = vec3(hsl.z); // Directly use lightness for gray
    // Chromatic color case
    else {
        float f2;
        ...
        // Calculate RGB components using hue rotations
