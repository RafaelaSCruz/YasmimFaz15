...
void main() {
    ...
    float alpha_ori = rgba.r;

        if (inputEdgeSmooth > EPS)
        {
            float alpha_ = 0.0;
            float L = min(1.0 / widthOffset, 1.0 / heightOffset) * 0.03;
            int r = 21;//int(inputEdgeSmooth * L);
            ...
            for (int i = kernel_size - 1; i >= 0; i--)
            {
                ...
                vec2 coordinate = uv0 + offset * 2. * inputEdgeSmooth;
                if (inputEdgeSmooth < .2) {
                    if (i > 23 || i < 19) continue;
                    coordinate = uv0 + offset * 8. * inputEdgeSmooth;
                }
                coordinate = clamp(coordinate, 0.0, 1.0);
                float pt_alpha = texture2D(smoothMapTexturex, coordinate).x;
                float w = normpdf(abs(float(i - kernel_size / 2)) / float(r) * 15., 4.);
                alpha_ += pow(pt_alpha, gamma) * w;
                weight += w;
            }
            alpha_ = clamp(alpha_ / weight, 0., 1.);
            alpha = pow(alpha_, 1. / gamma);
        }
    
    if (inputSpill > EPS)
    {
        rgba = processColor(rgba, vec4(0.0,0.0,0.0,1.0), alpha, inputSpill);
    }
    // gl_FragColor = rgba * alpha;
    // alpha = max(alpha, alpha_ori);
    gl_FragColor = vec4(alpha, 0, 0, 1);
    // gl_FragColor *= alpha_ori;

    if (enable_reverse > 0.5)
        gl_FragColor = vec4(vec3(1.0) - gl_FragColor.xyz, gl_FragColor.a);
}
